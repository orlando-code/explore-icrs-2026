...
import pandas as pd
from src.geocoding.capital_coords import coords_plausible_for_country, organisation_country_mismatch, resolve_capital_fallback, resolve_country_anchor_fallback
...
from src.geocoding.geocode import DEFAULT_OVERRIDES_PATH, _lookup_override, canonical_affiliation_key
from src.util.json_io import load_json
from src.data_paths import AFFILIATION_GEOCODES_CSV, AFFILIATION_GEOCODES_MANUAL_CSV
...
def load_geocode_overrides(path: Path | str=DEFAULT_OVERRIDES_PATH) -> dict[str, dict]:
    ...
    if _GEOCODE_OVERRIDES_CACHE is None:
        payload = load_json(Path(path), default={})
        _GEOCODE_OVERRIDES_CACHE = payload if isinstance(payload, dict) else {}
    return _GEOCODE_OVERRIDES_CACHE

def load_geocode_source_frames(path: Path | str=DEFAULT_GEOCODES_CSV, *, manual_path: Path | str=DEFAULT_MANUAL_GEOCODES_CSV) -> pd.DataFrame:
    ...
    combined['_priority'] = combined['status'].map(lambda value: {'OK': 3, 'FALLBACK': 2, 'IMPRECISE': 1}.get(str(value).strip(), 0))
...
def load_ok_geocodes(path: Path | str=DEFAULT_GEOCODES_CSV, *, manual_path: Path | str=DEFAULT_MANUAL_GEOCODES_CSV, include_fallback: bool=False) -> pd.DataFrame:
...
def _delegate_org_country_lookup() -> dict[str, tuple[str, str]]:
    ...
    for _, row in load_person_registry().iterrows():
        ...
        if not organisation:
            continue
        for name in (str(row.get('canonical_name') or '').strip(), *str(row.get('name_variants') or '').split(';')):
            name = name.strip()
            if not name:
                continue
...
def build_geocode_lookup(geocodes: pd.DataFrame) -> dict[str, Any]:
    ...
    org_country_counts = geocodes.groupby('organisation')['country'].nunique().to_dict() if not geocodes.empty else {}
    for row in geocodes.to_dict(orient='records'):
        organisation = str(row.get('organisation') or '').strip()
        country = str(row.get('country') or '').strip()
        affiliation = str(row.get('affiliation') or '').strip()
        record = {'latitude': float(row['latitude']), 'longitude': float(row['longitude']), 'formatted_address': str(row.get('formatted_address') or ''), 'query_used': str(row.get('query_used') or ''), 'geocode_level': 'institute', 'geocoded': True, 'organisation': organisation, 'country': country, 'affiliation': affiliation}
        if affiliation:
    ...
    return {'by_affiliation': by_affiliation, 'by_org_country': by_org_country, 'by_org': by_org}

def _override_hit(affiliation: str, override_lookup: dict[str, dict], *, organisation: str='', country: str='') -> dict[str, Any] | None:
    ...
    for candidate in candidates:
        ...
        if override is not None and override.get('latitude') is not None and (override.get('longitude') is not None):
            hit = {'latitude': float(override['latitude']), 'longitude': float(override['longitude']), 'formatted_address': '', 'query_used': str(override.get('query_used') or 'override'), 'geocode_level': str(override.get('geocode_level') or 'institute'), 'geocoded': True, 'organisation': organisation or parse_affiliation_parts(affiliation)[0], 'country': country or parse_affiliation_parts(affiliation)[1], 'affiliation': affiliation}
            return _accept_org_country_hit(hit, organisation=organisation or parse_affiliation_parts(affiliation)[0], country=country or parse_affiliation_parts(affiliation)[1], affiliation=affiliation)
    return None

def _capital_fallback_record(organisation: str, country: str, affiliation: str) -> dict[str, Any] | None:
    ...
    return {'latitude': lat, 'longitude': lon, 'formatted_address': f'{city}, {country}', 'query_used': query_label, 'geocode_level': 'country', 'geocoded': True, 'organisation': organisation, 'country': country, 'affiliation': affiliation}

def _accept_org_country_hit(hit: dict[str, Any] | None, *, organisation: str, country: str, affiliation: str) -> dict[str, Any] | None:
    if hit is None:
        return None
...
def _org_country_hit(organisation: str, country: str, *, affiliation: str, lookup: dict[str, Any]) -> dict[str, Any] | None:
    ...
    hit = _accept_org_country_hit(hit, organisation=organisation, country=country, affiliation=affiliation)
    if hit is not None:
        return hit
    hit = lookup['by_org_country'].get((organisation.casefold(), country.casefold()))
    hit = _accept_org_country_hit(hit, organisation=organisation, country=country, affiliation=affiliation)
    if hit is not None:
        return hit
    return anchor_hit or _capital_fallback_record(organisation, country, affiliation)

def _geocode_lookup_candidates(affiliation: str, *, organisation: str='', country: str='') -> list[str]:
    from src.geocoding.geocode import affiliation_display_name, resolve_affiliation_alias
    candidates: list[str] = []
    for value in (affiliation, resolve_affiliation_alias(affiliation), affiliation_display_name(affiliation), organisation, resolve_affiliation_alias(organisation), affiliation_display_name(organisation), f'{organisation}, {country}' if organisation and country else '', f'{resolve_affiliation_alias(organisation)}, {country}' if organisation and country else '', f'{affiliation_display_name(organisation)}, {country}' if organisation and country else ''):
        text = str(value or '').strip()
        if text and text not in candidates:
            candidates.append(text)
    return candidates

def resolve_geocode(affiliation: str, *, presenter: str='', lookup: dict[str, Any] | None=None, delegate_lookup: dict[str, tuple[str, str]] | None=None, overrides: dict[str, dict] | None=None) -> dict[str, Any] | None:
    if lookup is None:
    ...
    if not country and presenter:
        ...
        match = delegate.get(normalize_person_name(presenter)) or delegate.get(presenter.strip().casefold())
        if match:
    ...
    if lookup_org:
        ...
        if country:
            ...
            country_matches = [candidate for candidate in candidates if str(candidate.get('country') or '').casefold() == country_key]
            if len(country_matches) == 1:
                return country_matches[0]
        elif len(candidates) == 1 and (not organisation_country_mismatch(lookup_org, country)):
            return candidates[0]
    return None

def export_geocode_overrides_js(save_path: str | Path='js/geocode-overrides.js') -> Path:
    """Export data/geocode_overrides.json for runtime map pin correction."""
    from src.geocoding.geocode import affiliation_base_name, affiliation_display_name, load_affiliation_display_aliases
    ...
    for source, target in aliases.items():
        for candidate in (target, affiliation_display_name(target), affiliation_base_name(target)):
            override = overrides.get(candidate)
            if override is None or override.get('latitude') is None:
                continue
            add(source, float(override['latitude']), float(override['longitude']))
            break
    entries.sort(key=lambda item: str(item[0]).casefold())
    output_path = Path(save_path)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    body = f'/** Generated from data/geocodes/geocode_overrides.json – do not edit by hand. */\nexport const AFFILIATION_GEOCODE_OVERRIDE_ENTRIES = {json.dumps(entries, ensure_ascii=False, indent=2)};\n'
