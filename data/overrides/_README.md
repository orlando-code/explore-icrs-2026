...
**Local only (gitignored):** `check_in_overrides.csv` – manual fixes for Innovators check-in rows (country, organisation, names keyed by delegate ID). `delegate_id_match_review_*_merged.csv` – latest curated name ↔ official-ID links for offset verification.
